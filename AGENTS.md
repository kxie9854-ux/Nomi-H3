...
继续开发先读根目录 `HANDOFF.md` 和 `FORK.md`。不要把本仓库当成 `/Users/aoqimin/Desktop/Nomi` 的上游工作树。供应商模型雷达（`pnpm run radar:models`）对本 fork **opt-in**：只在用户明确要求扫 AutoDL.art / Codex 时才跑，不要每 session 自动打 apimart/kie。
