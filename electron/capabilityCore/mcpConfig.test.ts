...
describe('capabilityCore/mcpConfig', () => {
  ...
  it('packaged launchers never inherit development-only profile or renderer variables', () => {
    ...
    // Library/settings roots identify the running Nomi instance for every launcher kind;
    // only the Electron profile and renderer wiring are development-only.
    expect(entry.env?.NOMI_PROJECTS_DIR).toBe(path.join(homeDir, 'projects'))
    expect(entry.env?.NOMI_SETTINGS_DIR).toBe(homeDir)
